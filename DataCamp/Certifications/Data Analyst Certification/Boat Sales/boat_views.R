...
df <- df %>% slice_max(Number.of.views.last.7.days, n = 989)
...
brands <- brands %>% mutate(Arank = ntile(Avg, 5), Crank = ntile(CV, 5)) %>%
  arrange(desc(N),desc(Arank),Crank) %>%
  slice(-1)
...
models %>% arrange(desc(Avg)) %>% slice_max(Avg,n = 10) %>%
  ggplot() +
...
years %>% arrange(desc(Avg)) %>% slice_max(Avg,n = 10) %>%
  ggplot() +
...
material <- tab_avg(df$Material)
material$Material <- material$`{ ... }`
material <- material %>% mutate(Arank = ntile(Avg, 3), Crank = ntile(CV, 3)) %>%
  arrange(desc(Arank),Crank)

material$Crank <- as.factor(material$Crank)
material <- material %>% mutate(Level = case_when(Crank == "3" ~ "Volatile",
                                            Crank == "1" ~ "Stable",
                                            TRUE ~ "Regular"))

material %>% arrange(desc(Avg)) %>% filter(Material != "") %>%
  ggplot() +
  aes(x = reorder(Material, Avg), fill = Level, weight = Avg) +
  coord_flip() +
  geom_bar() +
  scale_fill_manual(
    values = c(`Volatile` = "#D0767E",`Stable` = "#175676",`Regular` = "#B9C0CA")) +
...
tab <- df %>% group_by(Location) %>% summarise(Posts = n(),
  ...
  arrange(desc(Posts), desc(AvgViews))
